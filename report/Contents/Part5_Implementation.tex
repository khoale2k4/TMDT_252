...
\begin{enumerate}
    \item \textbf{Nhóm Vận hành thời gian thực:} Bao gồm các thuộc tính như \texttt{status} (available, locked, booked) và \texttt{locked\_until} để hiện thực hóa cơ chế \textit{Optimistic Locking}, chống tình trạng Overbooking.
    \item \textbf{Nhóm Tài chính và Hậu cần:} Liên kết giữa \textbf{Bookings} với các thực thể \textbf{Payments}, \textbf{Transactions} và \textbf{Invoices} (Hóa đơn điện tử), hỗ trợ đối soát tự động và tích hợp MISA.
    \item \textbf{Nhóm Tương tác và Mở rộng:} Các thực thể \textbf{Reviews} (đánh giá), \textbf{Matchmaking} (ghép trận) và cấu hình \textbf{Pricing} (định giá động) được thiết kế tách biệt để có thể nâng cấp mà không ảnh hưởng đến lõi hệ thống.
\end{enumerate}
